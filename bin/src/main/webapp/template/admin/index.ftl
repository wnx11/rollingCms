...
$(function() {
	...
	$table.bootstrapTable({
		...
		columns: [
			...
			{field: 'id', visible:false, sortable: true, align: 'center'},
			...
			{field: 'depart', title: '部门', sortable: true, align: 'center'},
			{field: 'login_date', title: '最后登录日期', sortable: true, align: 'center'},
			{field: 'login_ip', title: '最后登录IP', sortable: true, align: 'center'},
...
function createAction() {
	...
	var addlayer=layer.open({
	  ...
	  area: ['600px', '600px'],
	  type: 2, 
	  content: '${base}/admin/addPage.jhtml',
	  end: function () {
        location.reload();
      }
	}); 
...
function updateAction(rows) {
	...
	if (rows.length == 0) {
		...
		var editlayer=layer.open({
		  ...
		  area: ['600px', '600px'],
		  type: 2, 
		  content: '${base}/admin/editPage.jhtml?id='+id,
		  end: function () {
	        location.reload();
	      }
		});
